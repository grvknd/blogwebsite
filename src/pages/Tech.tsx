...
export default function Tech() {
  ...
  const getThumb = (url?: string) => {
    if (!url) return "";
    // Handles: youtu.be/ID , youtube.com/watch?v=ID , youtube.com/watch?v=ID&...
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const filtered = all.filter((p) => p.section === "tech");
        setPosts(filtered);
      } catch (e) {
    ...
    <div style={{ padding: 20 }}>
      ...
      {!loading && posts.length === 0 && (
        <div style={{ textAlign: "center", padding: "40px 0" }}>
          <h3 style={{ color: "#94a3b8", marginBottom: "8px" }}>No tech posts yet</h3>
          <p style={{ color: "#64748b" }}>
            Stay tuned for the latest technology updates and tutorials!
          </p>
        </div>
      )}
